...
export default function WorkflowCreatorEarningsPage() {
  const queryClient = useQueryClient();
  const { toast } = useToast();
  const [payoutAmount, setPayoutAmount] = useState<string>("");
  
  const { data, isLoading } = useQuery({
    queryKey: ["workflow-my-earnings"],
    queryFn: async () => {
      const response = await apiRequest('GET', '/api/workflow-marketplace/my-earnings');
  ...
  const requestPayoutMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setPayoutAmount("");
      queryClient.invalidateQueries({ queryKey: ["workflow-my-earnings"] });
  ...
  return (
    <>
      <SEO
        title="Workflow Earnings - fxns"
        description="View your workflow sales, earnings, and analytics"
      />
        ...
        <div className="container mx-auto px-4 py-8 max-w-6xl">
          <div className="mb-8">
            <h1 className="text-3xl font-bold mb-2 flex items-center gap-2">
              ...
              Workflow Creator Earnings
            </h1>
            <p className="text-muted-foreground">
              Track your workflow sales and revenue
            </p>
          </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Your latest workflow purchases and earnings
              </CardDescription>
            </CardHeader>
            <CardContent>
              {sales.length === 0 ? (
                <div className="text-center py-12 text-muted-foreground">
                  ...
                  <p className="text-sm mt-2">
                    Publish your workflows to start earning!
                  </p>
                </div>
              ) : (
                    ...
                    >
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <h4 className="font-medium">{sale.workflow.name}</h4>
                        </div>
